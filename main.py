...
try:
    ...
    #     '0 Касперский Евгений Валентинович NULL eugenkasper@mail.ru secure123secure 0 1965-10-04'.split(),
    #     '1 Дуров Павел Валерьевич NULL durovmilliarder@mail.ru milliarder123milliarder 0 1984-10-10'.split(),
    #     '2 Элбакян Александра Асановна NULL alexandra@mail.ru sc1-hubt0p 1 1988-11-6'.split(),
    #     '3 Данилов Игорь Анатольевич NULL igorweb@mail.ru web123web 0 1964-4-22'.split(),
    #     '4 Скляров Дмитрий Витальевич NULL sclarovbaum@mail.ru mg3ubaum 0 1974-12-18'.split(),
    #     '5 Пянтковская Татьяна Алексеевна NULL tat2006@mail.ru gzynrjdcrfz17 1 2006-08-17'.split(),
    #     '6 Кузнецов Илья Евгеньевич NULL ilyanew@mail.ru 2007zxc2406 1 2007-06-24'.split()
    #
    # ]
    # a = 'INSERT INTO users VALUES '
    # for i in range(len(val)):
    #     a += f"({val[i][0]}, '{val[i][5]}', '{val[i][6]}', '{val[i][1]}', '{val[i][2]}', '{val[i][3]}', {val[i][7]}, NULL, '{val[i][8]}'), "
    # a = a.rstrip(', ')
    # print(a)
    #
    # cursor.execute(a)
...
#
    ...
    #
    # ]
    # a = 'INSERT INTO courses(id, title, description, cover, plan, price) VALUES '
    # for i in range(len(val)):
    #     a += f"({val[i][0]}, '{val[i][1]}', '{val[i][2]}', '{val[i][3]}', '{val[i][4]}', {val[i][5]}), "
    # a = a.rstrip(', ')
    #
    # cursor.execute(a)
...
#
    # val = [
    #     # (id, idCourse, idUser, title, descripton, datetime, duration, isComplete, file, commentFile)
    ...
    #     ['NULL'] * 100
    # ]
    # a = 'INSERT INTO lessons VALUES '
    # for i in range(100):
    #     a += f"({i}, {val[0][i]}, {val[1][i]}, '{val[2][i]}', '{val[3][i]}', {val[4][i]}, {val[5][i]}, {val[6][i]}, {val[7][i]}, {val[8][i]}), "
    # a = a.rstrip(', ')
    # print(a)
    #
    # cursor.execute(a)
...
#
    ...
    #     [15, 'Java']
    # ]
    # a = 'INSERT INTO tags(id, name) VALUES '
    # for i in range(16):
    #     a += f"({i}, '{val[i][1]}'), "
    # a = a.rstrip(', ')
    #
...
#
    ...
    # ]
    # a = 'INSERT INTO courseMentors VALUES '
    # for i in range(len(val[0])):
    #     a += f"({i}, {val[0][i]}, {val[1][i]}), "
    # a = a.rstrip(', ')
    #
    # cursor.execute(a)
...
#
    ...
    # ]
    # a = 'INSERT INTO tagsCourses VALUES '
    # for i in range(len(val[0])):
    #     a += f"({i}, {val[1][i]}, {val[0][i]}), "
    # a = a.rstrip(', ')
    #
    # cursor.execute(a)
...
#
    ...
    # ]
    # a = 'INSERT INTO materials(id, title, cover, url) VALUES '
    # for i in range(len(val)):
    #     a += f"({i}, '{val[i][0]}', '{val[i][1]}', '{val[i][2]}'), "
    # a = a.rstrip(', ')
    #
    # cursor.execute(a)
...
#
    ...
    #     ['6', '9', 'DATE(NOW())'],
    # ]
    # a = 'INSERT INTO soldCourses VALUES '
    # for i in range(10):
    #     a += f"({i}, {val[i][1]}, {val[i][0]}, {val[i][2]}), "
    # a = a.rstrip(', ')
    #
    # cursor.execute(a)
    ...
    a = """SELECT * FROM lessons WHERE "idCourse" = 0"""
    cursor.execute(a)
    data = cursor.fetchall()
    ans = []
    for i in range(10):
        a = f"""SELECT * FROM lessons WHERE "idCourse" = {i}"""
        cursor.execute(a)
        data = cursor.fetchall()
        json = []
        for j in data:
